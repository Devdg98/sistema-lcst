<template>
    ...
    <h2 class="titulo text-center mb-2">MOLETONS HUGOBOSS</h2>
 <v-toolbar class="toolbar" color="white">
      <v-bottom-sheet v-model="filtrosAtivos">
        ...
        <template v-slot:activator="{ props: activatorProps }">
          ...
          <v-sheet class="pa-0 rounded-0 bg-white" elevation="0" >
          <v-toolbar class="mt-2 d-flex bg-white justify-space-between align-center">
  ...
  >
    ...
    <v-card-actions class="px-4 pb-4">
        <!-- Botão que ativa o segundo bottom sheet (carrinho) -->
         <v-btn class="botaolacoste" block @click="adicionarAoCarrinho">
              Ir para Carrinho
            </v-btn>
...
const adicionarAoCarrinho = () => {
  if (!model.value || !modelTamanho.value) {
    alert('Selecione a cor e o tamanho antes de adicionar ao carrinho.')
    return
  }

  carrinho.adicionarItem(currentProductDescription.value, model.value, modelTamanho.value)
  dialogImagem.value = false // fecha visualização rápida
  dialogCarrinho.value = true // abre o carrinho
}
...
const items = [
  { title: 'Vestuário', href: '/hugo/vestuario', disabled: false },
  { title: 'Calçados', href: '/hugo/calcados', disabled: false },
  { title: 'Polos', href: '/hugo/polos', disabled: false },
  { title: 'Bonés', href: '/hugo/bones&chapeus', disabled: false },
  { title: 'Camisetas', href: '/hugo/camisetas&regatas', disabled: false },
  { title: 'Moletons & Jaquetas', href: '/hugo/moletons', disabled: false },
  { title: 'Calças & Bermudas', href: '/hugo/shorts&bermudas', disabled: false }, // <- este aqui é o último
];
...
const produtosFiltrados = computed(() => {
  return catalogo.produtos.filter(produto => {
    const categoriaMatch = produto.categoriaProduto === 'Moletons & Jaquetas';
    const marcaMatch = produto.marcaProduto === 'HugoBoss';
    ...
    return categoriaMatch && marcaMatch && corMatch && tamanhoMatch;
  });
});
...
.resultados {
  font-size: 10px;
  color: #000000; /* Verde claro para os resultados */
}
/* Botão FILTROS */
.btn-filtros {
  color: white;
