...
export const useBannerStore = defineStore('bannerCategorias', {
  ...
  actions: {
    ...
    async carregarProdutos() {
      this.loading = true;
      try {
        const { data } = await axios.get('https://backendgeral-147424face7e.herokuapp.com/banner');
        this.produtos = data;
        this.error = null;
      } catch (err) {
  ...
  try {
    const { data } = await axios.post(
      'https://backendgeral-147424face7e.herokuapp.com/banner/upload',
      formData,
  ...
  try {
    const { data } = await axios.post(
      'https://backendgeral-147424face7e.herokuapp.com/banner',
      produtoPayload
    );
...
async atualizarProduto(id, produtoPayload) {
  ...
  try {
    const { data } = await axios.put(
      `https://backendgeral-147424face7e.herokuapp.com/banner/${id}`,
      produtoPayload
    );
    ...
    async excluirProduto(id) {
      this.loading = true;
      try {
        await axios.delete(`https://backendgeral-147424face7e.herokuapp.com/banner/${id}`);
